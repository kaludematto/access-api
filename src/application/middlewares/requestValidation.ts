...
import { PayloadError } from '../helpers';
...
export const validateDTO = (requestToValidate: RequestValidate) => {
    return async function (req: Request, res: Response, next: NextFunction) {
        const allErrors = [];
        const validateKeys = Object.keys(requestToValidate) as [
            keyof RequestValidate,
        ];
        ...
        for (const validationKey of validateKeys) {
            ...
            if (dtoToValidate) {
                ...
                const output: any = plainToInstance(
                    dtoToValidate,
                    requestObjectToValidate,
                );
                ...
                if (errors.length > 0) {
                    ...
                    for (const errorItem of errors) {
                        errorTexts = errorTexts.concat(
                            errorItem.constraints || errorItem.children,
                        );
                    }
